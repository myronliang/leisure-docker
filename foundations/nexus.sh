...
if [ -z "$container_exist" ]; then
    ...
    echo "检查容器[${image_name}]不存在，初始化命令：docker run -d -p 8081:8081 -p 8082:8082 -p 8083:8083 --name ${image_alias} --restart=always --network bridge --ip 172.18.0.2 -v ${path}:/nexus-data ${image_name}"
    docker run -d -p 8081:8081 -p 8082:8082 -p 8083:8083 --name $image_alias --restart=always --network mynetwork --ip 172.18.0.2 -v $path:/nexus-data $image_name
else
    if [[ -z "$1" ]]; then
        ...
        echo "删除容器[${image_name}]成功" && exit 1
    fi
fi
